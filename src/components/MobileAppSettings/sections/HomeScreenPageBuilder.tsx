...
export function HomeScreenPageBuilder({ pathPrefix, getValue, updateField }: CustomFieldProps) {
  ...
  const fieldPath = useMemo(
    () => activeTab === 'guest' ? `${pathPrefix}.guestSections` : `${pathPrefix}.customerSections`,
  ...
  const copyToCustomerTab = useCallback((index: number) => {
    ...
    const customerSections = (getValue(`${pathPrefix}.customerSections`) as ContentBlock[] | undefined) ?? []
    ...
    updateField(`${pathPrefix}.customerSections`, [...customerSections, copiedSection])

    alert('Section copied to Customer tab!')
  }, [activeTab, sections, getValue, updateField, pathPrefix])
